...
class AyushSubCatagoryViewModel(
    ...
    private val _responseAyush = MutableLiveData<BaseResponseNamasteYoga<List<AyushSubCategoryList>>>()
    val responseAyush: LiveData<BaseResponseNamasteYoga<List<AyushSubCategoryList>>> = _responseAyush


    fun getAyushSubCategoryList(catId:String) {
        _page.value = page.value?.plus(1)
        launch {
            ayushCatagoryRepository.getAyushSubCategoryList(page.value?:CommonInt.One,catId)
                .with(scheduler)
                .subscribe({
