...
class TrainerFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        etCertificateNo.manageEditText(
            InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS, CommonInt.CertificateNoLength, getString(
                R.string.certificate_number
            )
        )
        ...
        buttonColorLayout.setOnClickListener {
            ...
            val certi = etCertificateNo.getMyEditText()
            if (PatternUtil.isNullOrEmpty(certi.text.toString())) {
                TextUtils.errorEmptyText(certi, getString(R.string.certificate_number))
                return@setOnClickListener
            }
            if (PatternUtil.isValidCertificate(certi.text.toString())) {
                TextUtils.errorValidText(certi, getString(R.string.certificate_number))
                return@setOnClickListener
            }
            ...
            val registerRequest = RegisterRequest().apply {
                ...
                roleId = UserRoleId
                zip = ZIP
                certificateNumber = certi.text.toString().trim()
...
//                stateId = try{citySelected?.state_id?.trim()?.toInt()}catch (e:Exception){0}
...
//                            val lat = "" + latLng?.latitude + "," + latLng?.longitude
                            ...
                            val lat = "${df.format(latLng?.latitude)},${df.format(latLng?.longitude)}"
