...
import com.np.namasteyoga.ui.eventModule.searchEvent.SearchEventActivity;
...
//public class FragmentEvents extends Fragment implements Rating {
public class FragmentEvents extends AppCompatActivity implements Rating, Rating2, OnClickItem<Event>, OnLocaleChangedListener {
...
//    AdapterEventPager pastEventAdapterEventPager;
    ...
    private PastEventDialogFragment pastEventDialogFragment = new PastEventDialogFragment();
...
//        currentEventAdapterEventPager = new AdapterEventPager(this,currentEventList, FragmentEvents.this,sharedPreferences,false);
        currentEventAdapterEventPager = new EventListAdapter(currentEventList, FragmentEvents.this, sharedPreferences, false, 0, this, false);
...
//        upcomingEventAdapterEventPager = new AdapterEventPager(this, upcomingEventList, FragmentEvents.this,sharedPreferences);
        upcomingEventAdapterEventPager = new EventListAdapter(upcomingEventList, FragmentEvents.this, sharedPreferences, false, 1, this, false);
        ...
        pastEventAdapterEventPager = new EventListAdapter(pastEventList, FragmentEvents.this, sharedPreferences, true, 2, this, false);
...
//        ivSearch.setVisibility(View.GONE);
        ivSearch.setOnClickListener(view -> {
            //Constants.currentYogaType = "event";
            startActivity(new Intent(FragmentEvents.this, SearchEventActivity.class));

        });

        btnTodayEvent.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        btnUpcomingEvent.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                setNotifyButtonVisibility(false);
                llevents.setVisibility(View.VISIBLE);
        ...
        btnPastEvent.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ...
                btnPastEvent.setTextColor(ContextCompat.getColor(FragmentEvents.this, R.color.white));
...
//            progressDialog = ConstUtility.getProgressDialog(getActivity(), "Getting Events...");
            ...
            JsonObjectRequest req = new JsonObjectRequest(BuildConfig.BASE_URL + C.API_EVENT_LIST, jsonObj,
...
//        if (ConstUtility.isNetworkConnectivity(getActivity())) {
        if (ConstUtility.isNetworkConnectivity(this)) {
            ...
            if (Util.INSTANCE.getCitySelected() != null) {
               ...
               /* JsonObjectRequest req = new JsonObjectRequest(C.BASE_URL+C.API_EVENT_LIST, jsonObj,
                        new Response.Listener<JSONObject>() {
                            @Override
                            public void onResponse(JSONObject response) {
                                try {
//                            utils.hideDialog();
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        tvSecondtext.setText(R.string.eventsecondtext);
                        hideUI();
                        Toast.makeText(FragmentEvents.this, getString(R.string.something_went_wrong), Toast.LENGTH_LONG).show();
                ...
                RetryPolicy policy = new DefaultRetryPolicy(C.API_TIME_OUT, DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
                req.setRetryPolicy(policy);
                requestQueue = Volley.newRequestQueue(FragmentEvents.this);
                requestQueue.add(req);*/
            }
        } else {
            Toast.makeText(FragmentEvents.this, getString(R.string.no_internet_connection), Toast.LENGTH_LONG).show();
        }
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            } /*else if (eventResponse.getStatus().equals(C.NP_INVALID_TOKEN) || eventResponse.getStatus().equals(C.NP_TOKEN_EXPIRED) || eventResponse.getStatus().equals(C.NP_TOKEN_NOT_FOUND)) {
                                SharedPreferencesUtils.INSTANCE.setUserDetails(sharedPreferences, null);
                                Toast.makeText(FragmentEvents.this, eventResponse.getMessage(), Toast.LENGTH_LONG).show();
                                finish();
                                Intent intent = new Intent(FragmentEvents.this, LoginActivity.class);
...
//                            utils.hideDialog();
    ...
    private void showLoginButtton() {
        if (SharedPreferencesUtils.INSTANCE.getUserDetails(sharedPreferences) == null) {
            Animation slideFromRight = AnimationUtils.loadAnimation(FragmentEvents.this, R.anim.slide_from_right);
            llLogin.setVisibility(View.VISIBLE);
    ...
    @OnClick({R.id.ivBack, R.id.rlSubmit, R.id.llcreateevent, R.id.llLogin})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.ivBack:
                ConstUtility.hideKeyBoard(FragmentEvents.this);
                onBackPressed();
                break;
            case R.id.rlSubmit:
                if (SharedPreferencesUtils.INSTANCE.getUserDetails(sharedPreferences) != null) {
...
//                startActivity(new Intent(this, CreateEventActivity.class));
    ...
    private void doNotify() {
        ...
        notifyRequest.setName(userDetail.getName());
        notifyRequest.setPhone(userDetail.get_phone());
        notifyRequest.setEmail(userDetail.get_email());
...
//        notifyRequest.setType("center");
        ...
        progressDialog = ConstUtility.getProgressDialog(FragmentEvents.this, getString(R.string.sending_request_));
        progressDialog.show();
        if (ConstUtility.isNetworkConnectivity(FragmentEvents.this)) {
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                ...
                                if (notifyResponse.getStatus().equals(C.NP_STATUS_SUCCESS)) {
                                    Toast.makeText(FragmentEvents.this, R.string.requested_for_this_location,
                                            Toast.LENGTH_LONG).show();
                                    onBackPressed();
                                } else if (notifyResponse.getStatus().equals(C.NP_STATUS_FAIL)) {
                                    Toast.makeText(FragmentEvents.this, notifyResponse.getMessage(),
                                            Toast.LENGTH_LONG).show();
                                } else {
                                    Toast.makeText(FragmentEvents.this, R.string.already_requested_for_this_location,
                                            Toast.LENGTH_LONG).show();
                                }
            ...
            Toast.makeText(FragmentEvents.this, getString(R.string.no_internet_connection),
                    Toast.LENGTH_LONG).show();
        }
    }
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                ...
                                if (ratingResponse.getStatus().equals(C.NP_STATUS_SUCCESS)) {
                                    Toast.makeText(FragmentEvents.this, R.string.thanks_for_rating, Toast.LENGTH_SHORT).show();
                                    dialog.dismiss();
...
//                                        pastEventAdapterEventPager.getItem(pos).setRating(Float.parseFloat(ratingResponse.getRatingData().getRating()));
                                    ...
                                    Toast.makeText(FragmentEvents.this, ratingResponse.getMessage(), Toast.LENGTH_SHORT).show();
                                    dialog.dismiss();
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
            ...
            RetryPolicy policy = new DefaultRetryPolicy(C.API_TIME_OUT, DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            req.setRetryPolicy(policy);
            RequestQueue requestQueue = Volley.newRequestQueue(FragmentEvents.this);
            requestQueue.add(req);
        } else {
...
//        etPhone.setText("");
    ...
    public void ratingDialogUpdate(Event event) {
        final Dialog dialog = new Dialog(FragmentEvents.this);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setCancelable(false);
        dialog.setContentView(R.layout.rating_dialog_2);
        // Animation slideUp = AnimationUtils.loadAnimation(getActivity(), R.anim.slide_in_bottom);
...
//        EditText etFullName = dialog.findViewById(R.id.etFullName);
    ...
    private void hideKeyboard(Dialog dialog) {
        if (dialog.getCurrentFocus() != null) {
            InputMethodManager inputManager = (InputMethodManager) Objects.requireNonNull(FragmentEvents.this).getSystemService(Context.INPUT_METHOD_SERVICE);
    ...
    public void onClick(int position, @org.jetbrains.annotations.Nullable Event event) {
        if (event != null) {
            switch (position) {
                ...
                case 2:
                    openPastEventBottomSheet(event);
//                    if (SharedPreferencesUtils.INSTANCE.getUserDetails(sharedPreferences) != null) {
...
//                    }
    ...
    private void openRoundedBottomSheet(Event event) {
        ...
        if (fragmentCreateEvents != null && fragmentCreateEvents.isVisible()) {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(() -> myRoundedBottomSheet.setData(event, eventType), 300);
        }
    }

    private void openPastEventBottomSheet(Event event) {
        FragmentCreateEvents fragmentCreateEvents = (FragmentCreateEvents) getSupportFragmentManager().findFragmentByTag(C.TAG_FRAGMENT_CREATE_EVENT);

        if (fragmentCreateEvents != null && fragmentCreateEvents.isVisible()) {
        } else {
            if (pastEventDialogFragment.isAdded() || pastEventDialogFragment.isVisible()) {
                pastEventDialogFragment.dismiss();
            }
            pastEventDialogFragment.show(getSupportFragmentManager(), pastEventDialogFragment.getTag());
            new Handler(Looper.getMainLooper()).postDelayed(() -> pastEventDialogFragment.setData(event), 300);
        }
    }
    ...
    public void openEditEvent(Event event) {
        ...
        fragmentTransaction.replace(R.id.rlEvent, fragment);
    ...
    public void ratingDialog(@org.jetbrains.annotations.Nullable String pos, int position) {
        if (SharedPreferencesUtils.INSTANCE.getUserDetails(sharedPreferences) != null)
            ratingDialogUpdate(pastEventList.get(position));
        else {
            startActivityForResult(new Intent(this, LoginActivity.class), 100);
        }
    }
